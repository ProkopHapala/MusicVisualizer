...
void main( ){
    ...
    vec2 uv = (gl_FragCoord.xy - 0.5 * iResolution.xy) / iResolution.y;//Center Origin, Remap to -0.5 to +0.5, and square using aspect ratio.
    ...
    //col += texture(iChannel0, uv*0.1 - iTime * 0.1).rgb;
    
    //gl_FragColor = vec4(sin(uv*30),1.,1.) + vec4(col, 1.0);
    ...
    //vec3 col = texture(iChannel0, (uv+1.0)*0.5 ).rgb;
    //gl_FragColor = vec4(col, 1.0);  
    
    
    vec2 pos   = ( gl_FragCoord.xy / iResolution.xy );
    vec2 pixel = iMouse*0.0001 + (1./iResolution);
    pixel += sin(pos*10.2);
    vec4 sum;
