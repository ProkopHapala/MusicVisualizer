...
// From  Hyperbolic Truchet tiles   https://www.shadertoy.com/view/3llXR4
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    const vec3 unitCircle = vec3(0, 0.0, 1);
    ...
    if (dot(uv, uv) < 1.) {
        ...
        if (dot(m, m) < 0.95) {
            ...
            float phase = t  + Const.y*5.0;
            float r = 0.5*smoothstep(0.0, 4.0, phase);
            m = r*vec2(cos(phase), sin(phase));
            scroll = true;
        } else {
            m = vec2(0);
        }
        ...
        vec2 seed = floor(12.*ctr + 0.5) + 19. + Const.y*0.01;
