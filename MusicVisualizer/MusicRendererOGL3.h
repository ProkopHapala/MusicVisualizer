...
class RenderStackManager{ public:
    ...
    void loadShader( const char* vertName, const char* fragName, bool bFile = true ){
        //Shader* sh = new Shader( "resources/shaders/texture3D.glslv" , "resources/shaders/texture.glslf" , true );
    ...
    */
    ...
    void loadShaders(){
        const char prefix[]{"resources/shaders/"};
        char vertPath[1024];
        for(int i=0; i<shaderNames.size(); i++){
            sprintf(vertPath,"%s%s.glslf",prefix, shaderNames[i].c_str() );
            printf( "Loading %s \n", vertPath );
            loadShader( "resources/shaders/texture3D.glslv", vertPath );
        }
    }
